...
```
Yes because the R^2 value is greater and thus has greater variability attributed to it
...
ggplot(data = m9, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")

ggplot(data = m9, aes(x = .resid)) +
  geom_histogram(binwidth = 25) +
  xlab("Residuals")

ggplot(data = m9, aes(sample = .resid)) +
  stat_qq()
```

1. The plot of the residuals indicates some linearity.
...
3. Histogram is fairly symmetric thus is appropriate to deem the normal distribution condition met.

4. Based on the residuals plot the variability of points around the least squares line remains roughly constant so the condition constant variability has been met.
