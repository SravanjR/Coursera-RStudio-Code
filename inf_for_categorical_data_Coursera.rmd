...
sum(us12$response == "atheist") / count(us12)
```
...
It is an interval in which the value will fall with a % probability
...
af12 <- subset(atheism, atheism$nationality == "Japan" &
                   atheism$year == "2012")
inference(y = response, data = af12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
0.3069 - 0.281

```
Spain Margin of Error: 0.09 - 0.0734 = 0.0166
Japan Margin of error: 0.3069 - 0.281 = 0.0259
...
inference(y = response, data = es, as.factor(es$year),
...
inference(y = response, data = us, as.factor(us$year),
          statistic = "proportion", type = "ht", method = "theoretical",
          null = 0, alternative = "twosided", success = "atheist")
```
...
MER <- 0.01
p <- 0.5
# MER = 1.96 * SE for 95% confidence margin
# SE = sqrt(p*(1 − p)/n)
# MER = 1.96 * sqrt(p*(1 − p)/n)
# MER^2 = 1.96^2 * p*(1 − p)/n
# n = 1.96^2 * p*(1 − p)/MER^2
1.96^2 * p*(1 - p)/MER^2
```
