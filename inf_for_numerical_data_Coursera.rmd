...
ggplot(data = nc, mapping = aes(x= nc$habit, y= nc$weight)) + geom_boxplot() + labs(x = "Smoking Habits", y = "Birth weights")
```
...
We reject the null Hypothesis in favor of the alternative due to the low p-value
</div>
...
p_value = 0.1711 which is rather high and thus we do not reject the null hypothesis. 
...
```
34 is the age cutoff for mature and young mothers. The average age of child birth overall is around 30 years of age. Leaning to the older demographic for a cutoff is ideal. 
